...
class Save {
  ...
  static void saveToDisk(ImageProvider? provider, String filename) async {
    ...
    if (provider is CachedNetworkImageProvider) {
      ...
      // ignore: deprecated_member_use
      print(_cache.url);
      DefaultCacheManager().getFile(_cache.url).listen((stream) async {
        // Directory? appDocDirectory = await (getExternalStorageDirectory());
        // filename = filename.replaceAll(RegExp(r'[^\d]'), '');
        // String dir = appDocDirectory!.path + '/nedo/photos' + filename;
        // await Dio().download(_cache.url, dir);
        // print("dowloaded");
        _save(Directory directory) {
          stream.file.readAsBytes().then((bytes) async {
  ...
  static Directory? await(Future<Directory?> externalStorageDirectory) {}
}
