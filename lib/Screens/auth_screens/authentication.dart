...
class _AuthenticateState extends State<Authenticate> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (passcodeVisible()) {
        widget.type == AuthenticationType.passcode ? _showLockScreen() : _biometricAuthentication();
      }
    });
  }
  ...
  bool passcodeVisible() {
    ...
    return (passcodeTries <= 3 ||
        DateTime.now().isAfter(lastTried.add(Duration(minutes: math.pow(2, passcodeTries - 3) as int))));
  }
  ...
  _onPasscodeEntered(String enteredPasscode) {
    if (enteredPasscode.length == 4) {
      bool isValid = Fiberchat.getHashedAnswer(enteredPasscode) == widget.passcode;
      ...
      if (isValid) {
        ...
        widget.prefs.setInt(Dbkeys.lastAttempt, DateTime.now().millisecondsSinceEpoch);
        if (passcodeTries > 3) {
          Fiberchat.toast('Try after ${math.pow(2, passcodeTries - 3)} minutes');
  ...
  _biometricAuthentication() {
    LocalAuthentication()
        .authenticate(biometricOnly: true, localizedReason: widget.caption!, useErrorDialogs: true)
        .then((res) {
      if (res == true) {
